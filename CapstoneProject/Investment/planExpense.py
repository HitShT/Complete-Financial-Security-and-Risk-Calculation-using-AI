...
class planExpense:
    def __init__(self,amount,duration):
        ...
        for i in self.portfolio:
            self.temp[i] = self.portfolio[i][0]*self.portfolio[i][1]

        self.totalEquity = sum([self.temp[i] for i in self.temp])
        ...
        if self.remaining > 0:
            self.portfolio["Savings"] = [self.amountMonthlySave(self.remaining,self.convertYearlyMonthlyReturn(3)),0]
